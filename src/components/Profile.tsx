...
import { CheckCircle2, MapPin, Heart, MessageCircle, Camera, Edit2, Save, X, Heart as HeartIcon, MessageCircle as MessageCircleIcon } from 'lucide-react';
...
const Profile: React.FC<ProfileProps> = ({ user, userPosts, onUpdateUser, onPostClick, onViewStories, onLikePost, onCommentPost, onSharePost, onLogout }) => {
  ...
  return (
    <div className="pb-20">
      ...
      <div className="text-center mb-6">
        <div className="relative inline-block">
          ...
          <button
            ...
            className="absolute bottom-0 right-0 bg-indigo-600 p-2 rounded-full hover:bg-indigo-700"
          >
          ...
          />
          {user.isVerified && (
            <div className="absolute -bottom-1 -right-1 bg-gradient-to-br from-indigo-500 to-purple-500 p-1 rounded-full">
        ...
        {isEditing ? (
          <div className="mt-4 space-y-3 max-w-xs mx-auto">
            <input 
              ...
              placeholder="Prénom" 
              className="w-full bg-gray-100 rounded-lg px-4 py-2 text-sm outline-none border border-gray-300"
            />
            <input 
            ...
            <textarea
              ...
              placeholder="Bio"
              className="w-full bg-gray-100 rounded-lg px-4 py-2 text-sm outline-none border border-gray-300 min-h-20"
            />

            <div className="flex gap-2">
              <button onClick={handleSave} className="flex-1 bg-indigo-600 py-2 rounded-lg text-sm font-medium text-white flex items-center justify-center gap-2">
                <Save className="w-4 h-4" /> Enregistrer
              </button>
              <button onClick={() => setIsEditing(false)} className="flex-1 bg-gray-300 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2">
          ...
          <div className="mt-4">
            ...
            <button 
              ...
              className="mt-3 bg-gray-100 px-6 py-2 rounded-lg text-sm font-medium flex items-center gap-2 mx-auto"
            >
      ...
      {/* Posts Grid */}
      <h3 className="font-bold mb-3 text-gray-900">Mes Posts</h3>
      {userPostsList.length === 0 ? (
        <p className="text-center text-gray-500 py-8">Aucun post</p>
      ) : (
        <div className="grid grid-cols-3 gap-1">
          {userPostsList.slice(0, 9).map((post) => (
            <div key={post.id} className="aspect-square bg-gray-100 rounded-lg overflow-hidden cursor-pointer relative" onClick={() => onPostClick(post.id)}>
              <img src={post.contentUrl} className="w-full h-full object-cover" alt="" />
              <div className="absolute inset-0 bg-black/30 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <span className="text-white text-xs flex items-center gap-1">
                  <Heart className="w-3 h-3 fill-current" /> {post.likes?.length || 0}
                </span>
                <span className="text-white text-xs flex items-center gap-1">
