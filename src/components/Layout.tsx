...
const Layout: React.FC<LayoutProps> = ({ children, user, theme, setTheme, notifications, onClearNotifications, onNotificationClick }) => {
  ...
  return (
    <div className="min-h-screen relative transition-colors duration-300 overflow-x-hidden bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950">
      <header className="fixed top-0 z-[60] w-full glass px-4 py-2.5 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold tracking-tight hidden sm:inline bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">VISION</span>
          </Link>
        </div>

        <div className="flex items-center gap-1">
          <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} className="p-2.5 hover:bg-white/10 rounded-xl transition-all">
            {theme === 'light' ? <Moon className="w-5 h-5 text-zinc-600" /> : <Sun className="w-5 h-5 text-yellow-400" />}
          </button>

          <div className="relative" ref={notifRef}>
            <button onClick={() => setNotifOpen(!notifOpen)} className="p-2.5 hover:bg-white/10 rounded-xl transition-all relative">
            ...
            {notifOpen && (
              <div className="absolute top-full right-0 mt-3 w-80 glass rounded-3xl shadow-2xl border border-white/10 overflow-hidden z-[100]">
                <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
                  ...
                  <button onClick={onClearNotifications} className="text-[10px] text-zinc-500 hover:text-white transition-colors">Effacer</button>
                </div>
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div key={n.id} onClick={() => { onNotificationClick?.(n); setNotifOpen(false); }} className={`p-4 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer ${!n.read ? 'bg-indigo-500/5' : ''}`}>
                        ...
                        <p className="text-[10px] text-zinc-500 mt-1">{formatTimeAgo(parseInt(n.timestamp))}</p>
                      </div>
                    ))
                  )}
                </div>
      ...
      <aside className="hidden md:flex flex-col w-64 fixed left-0 top-0 bottom-0 glass p-4 pt-20 z-[50] border-r border-white/10">
        <nav className="space-y-1 flex-1">
          {navItems.map((item) => (
            <Link key={item.path} to={item.path} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${isActive(item.path) ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-500/25' : 'text-zinc-400 hover:bg-white/5 hover:text-white'}`}>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 glass flex items-center justify-around z-50 border-t border-white/10">
        {navItems.slice(0, 5).map((item) => (
          <Link key={item.path} to={item.path} className={`flex items-center justify-center w-14 h-14 rounded-xl transition-all ${isActive(item.path) ? 'text-indigo-400' : 'text-zinc-500'}`}>
