    ...
    <div className="flex gap-3 overflow-x-auto pb-3 scrollbar-hide">
      ...
      {stories.map((story) => (
        <div key={story.id} className="flex-shrink-0 flex flex-col items-center gap-1 cursor-pointer" onClick={() => onViewStory(story.user.id)}>
          <div className={`w-16 h-16 rounded-full p-[2px] ${story.viewed ? 'bg-gray-300' : 'bg-gradient-to-br from-rose-500 to-orange-500'}`}>
            <div className="w-full h-full rounded-full overflow-hidden border-2 border-white">
              <img src={story.imageUrl || story.user.avatar} className="w-full h-full object-cover" alt="" />
            </div>
          </div>
          <span className="text-[10px] text-gray-500 truncate w-16 text-center">{story.user.name}</span>
        </div>
      ))}
    </div>
  );
